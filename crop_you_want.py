...
def get_args():
    ...
    parser.add_argument('--no_sub_dir', action="store_true", help='Do not create additional subdirectories, take effect by set --no_sub_dir.')
    parser.add_argument('--no_save_score', action="store_true", help='Do not save score at image name, take effect by set --no_save_score.')
    parser.add_argument('--square_crop', action="store_true", help='Crop the image into a square, take effect by set --square_crop.')
...
def main():
    ...
    for IMAGE_PATH in image_path_list:
        ...
        for i in sorted_idx:
            ...
            counter[label] += 1
            crop_image_name = f"{label}_{counter[label]}"
            if not args.no_save_score:
                crop_image_name += f"_{score}"

            if args.square_crop:
                center_xy = ((box[0]+box[2])/2, (box[1]+box[3])/2)
                long_side, short_side = max((box[2]-box[0]), (box[3]-box[1])), min((box[2]-box[0]), (box[3]-box[1]))
                for side, square_name in zip([long_side, short_side], ["_circumscribed_square", "_inscribed_square"]):
                    distance = np.round(side/2)
                    x1 = int(max(center_xy[0]-distance, 0))
                    y1 = int(max(center_xy[1]-distance, 0))
                    x2 = int(min(np.ceil(center_xy[0]+distance), w))
                    y2 = int(min(np.ceil(center_xy[1]+distance), h))
                    crop_img = image_source[y1:y2, x1:x2]
                    cv2.imwrite(os.path.join(output_dir_for_a_image, (crop_image_name + square_name + file_extension)), crop_img)
            else:
